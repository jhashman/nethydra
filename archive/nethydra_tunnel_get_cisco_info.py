...
def main():
	try:
		...
		with open(nyap.nethydra_input) as csvfile:
			...
			for row in reader:
				...
				try:
					net_connect = connect.tunnel(ip, port, device_type, nyap.username1, nyap.password1, nyap.enable_pass, nyap.ssh_config_file)
					con_log.info('{0} - CONNECTION SUCCESSFUL'.format(ip))
				except Exception:
					con_log.warn('{0} - CONNECTION FAILED'.format(ip))
					try:
						net_connect = connect.tunnel(ip, port, device_type, nyap.username2, nyap.password2, nyap.enable_pass, nyap.ssh_config_file)
